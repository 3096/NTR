...
typedef u32 (*FSReadTypeDef)(u32 a1, u32 a2, u32 a3, u32 a4, u32 buffer,
                             u32 size);
...
u32 ScreenshotTopHotkey = PAD_L | PAD_R;
...
u32 initValuesFromHomeMenu() {
    ...
    copyRemoteMemory(CURRENT_PROCESS_HANDLE, &t, hProcess, (void*)0x00200000,
                     4);
    if (t == 0xe59f80f4) {
...
u32 HomeFSReadCallback(u32 a1, u32 a2, u32 a3, u32 a4, u32 buffer, u32 size) {
    u32 ret;
    ret = ((FSReadTypeDef)((void*)HomeFSReadHook.callCode))(a1, a2, a3, a4,
                                                            buffer, size);
    if (size == 0x36C0) {
...
u32 isFileExist(char* fileName) {
    ...
    ret = FSUSER_OpenFileDirectly(fsUserHandle, &hFile, sdmcArchive, testPath,
                                  1, 0);
    if (ret != 0) {
        return 0;
    }
...
void setCpuClockLock(int v) { cpuClockLockValue = v; }
...
u32 HomeSetMemorySizeCallback(u32 size) {
    ...
    u32 ret;
    ret =
        ((SetMemorySizeTypedef)((void*)HomeSetMemorySizeHook.callCode))(size);
    return ret;
}
...
void threadStart() {
    ...
    rtInitHook(&HomeFSReadHook, ntrConfig->HomeFSReadAddr,
               (u32)HomeFSReadCallback);
    rtEnableHook(&HomeFSReadHook);
    rtInitHook(&HomeCardUpdateInitHook, ntrConfig->HomeCardUpdateInitAddr,
               (u32)HomeCardUpdateInitCallback);
    ...
    // disp(100, 0x100ff00);
    while (1) {
        u32 down_keys = getKey();
        if (down_keys == ScreenshotHotkey && ScreenshotTopHotkey != 0) {
            do_screen_shoot(true);
        } else if (down_keys == ScreenshotTopHotkey && ScreenshotHotkey != 0) {
            do_screen_shoot(false);
        }
        if (down_keys == NTRMenuHotkey) {
...
void initFromSoc() { remotePlayMain(); }
...
int main() {
    ...
    if (StartMode == 1) {
        ...
        if (currentPid == ntrConfig->HomeMenuPid) {
            ...
            svc_createThread(&handle, (void*)threadStart, 0,
                             &threadStack[(STACK_SIZE / 4) - 10], 0x10, 1);
        } else {
            threadStack = (u32*)((u32)NS_CONFIGURE_ADDR + 0x1000);
            if ((isInDebugMode()) ||
                (g_nsConfig->startupCommand == NS_STARTCMD_DEBUG)) {
                svc_createThread(&handle, (void*)startupFromInject, 0,
                                 &threadStack[(STACK_SIZE / 4) - 10], 0x3f,
                                 0xFFFFFFFE);
                svc_sleepThread(1000000000);
            }

...
int sdf_open(char *filename, int mode) {
    ...
    retv = FSUSER_OpenFileDirectly(fsUserHandle, &hd, sdmcArchive, filePath,
                                   mode, 0);
    if (retv < 0) {
        return retv;
    }
...
void rev_image(u8 *dst, int width, int height, u8 *src, int src_pitch,
               int format) {
    int x, y, bpp;
    u8 *dp, *sp;
    u32 px;
...
void do_screen_shoot(bool both_screens) {
    u32 tl_fbaddr[2];
    u32 bl_fbaddr[2];
    u32 current_fb;
    ...
    current_fb &= 1;
    kmemcpy((void *)(workingBuffer + 0x50000), (void *)tl_fbaddr[current_fb],
            240 * 400 * 3);

    rev_buf = workingBuffer + sizeof(struct BitmapHeader);
    rev_image(rev_buf, 400, 240, workingBuffer + 0x50000, tl_pitch, tl_format);
    xsprintf(name, "/Screenshots/Screenshot_%s-%03d_top.bmp", ssTimeStr,
             ssMsec);
    ...
    if (both_screens) {
        ...
        current_fb &= 1;
        kmemcpy((void *)(workingBuffer + 0x50000),
                (void *)bl_fbaddr[current_fb], 240 * 320 * 3);

        rev_buf = workingBuffer + sizeof(struct BitmapHeader);
        rev_image(rev_buf, 320, 240, workingBuffer + 0x50000, bl_pitch,
                  bl_format);
        xsprintf(name, "/Screenshots/Screenshot_%s-%03d_bot.bmp", ssTimeStr,
                 ssMsec);
...
u32 fastCopyMemory(Handle hDst, u32 ptrDst, Handle hSrc, u32 ptrSrc,
                   u32 size) {
    u32 ret = 0;
...
u32 loadSaveVRAM(int fd, int isLoad) {
    ...
    svc_invalidateProcessDataCache(CURRENT_PROCESS_HANDLE, VRAM_ADDR,
                                   VRAM_SIZE);
...
u32 saveRegion(Handle hProcess, u32 addr, int fd, u32 offsetInFile, u32 size) {
    ...
    while (off < size) {
        ...
        ret = fastCopyMemory(CURRENT_PROCESS_HANDLE, image_buf, hProcess,
                             addr + off, currentRead);
        if (ret != 0) {
            return ret;
        }
...
u32 loadRegion(Handle hProcess, u32 addr, int fd, u32 offsetInFile, u32 size) {
    ...
    while (off < size) {
        ...
        ret = fastCopyMemory(hProcess, addr + off, CURRENT_PROCESS_HANDLE,
                             image_buf, currentRead);
        if (ret != 0) {
            return ret;
        }
        off += currentRead;
...
u32 saveAddrMap[] = {0x00100000, 0x08000000, 0x14000000, 0x30000000};
...
u32 instantSave(int id, int isLoad) {
    ...
    showDbg(
        isLoad ? "Load %d OK, GamePID: %08x." : "Save %d OK, GamePID: %08x.",
        id, gamePid);
final:
    if (fd > 0) {
        sdf_close(fd);
...
int checkBacklightSupported() {
    ...
    u32 ret;
    u8 buf[16] = {0};
    static u8 desiredHeader[16] = {0x30, 0x40, 0x2D, 0xE9, 0x00, 0x40,
                                   0xA0, 0xE1, 0x01, 0x5C, 0x80, 0xE2,
                                   0x8C, 0x01, 0xD0, 0xE5};
    ...
    copyRemoteMemory(getCurrentProcessHandle(), buf, hGSPProcess, gspPatchAddr,
                     16);
...
u32 backlightMenu() {
    ...
    while (1) {
        ...
        print(plgTranslate("Press Up/Down/Left/Right to adjust."), 10, 220, 0,
              0, 255);
        updateScreen();
        u32 key = waitKey();
        if (key == BUTTON_DU) {
...
u32 nightShiftUi() {
    ...
    entries[8] = plgTranslate(
        "Hint: Must be enabled before starting game. Will set CPU to "
        "L2+804MHz on New3DS.");
    while (1) {
        blank(0, 0, 320, 240);
...
int screenshotMain() {
    ...
    plgRegisterMenuEntry(1, plgTranslate("Real-Time Save (Experimental)"),
                         instantSaveMenu);

    nsDbgPrint("fsUserHandle: %08x\n", fsUserHandle);

    if (ntrConfig->isNew3DS) {
        plgRegisterMenuEntry(1, plgTranslate("CPU Clock (New3DS Only)"),
                             cpuClockUi);
    }
